...
resource "aws_route_table" "public" {
  count = local.create_public_subnets ? (var.create_multiple_public_route_tables ? local.len_public_subnets : 1) : 0
  ...
  tags = merge(
    {
      Name = var.create_multiple_public_route_tables ? format(
        "${var.name}-${var.public_subnet_suffix}-rt-%s",
        element(var.azs, count.index)) : "${var.name}-${var.public_subnet_suffix}-rt"
    },
    var.tags,
  ...
  tags = merge(
    {
      Name = var.single_nat_gateway ? "${var.name}-${var.private_subnet_suffix}-rt" : format(
        "${var.name}-${var.private_subnet_suffix}-rt-%s", element(var.azs, count.index))
    },
    var.tags,
